...
# 向量化的 RK4 方法
@njit
def rk4(x0, k_func, dt, num_steps):
    x = x0
    x_values = np.zeros(num_steps)  # 存储每个时间步的 x 值
    for i in range(num_steps):
        t = i * dt
        r_value = 0.47  # 固定 r 值
        k = k_func(t)  # 计算当前时间的 k(t)
...
for j in range(len(x0_values)):
    all_x_values[j] = rk4(x0=x0_values[j], k_func=k_func, dt=dt, num_steps=num_steps)
...
plt.xlabel('Time')
plt.ylabel('x')
plt.title('Dynamics of x over time with r fixed and k varying')
plt.axhline(y=2.36, color='grey', linestyle='--', label='x = 2.32')  # 添加参考线
plt.grid()
plt.legend(["x > 2.36", "x ≤ 2.36"], loc='upper left')
